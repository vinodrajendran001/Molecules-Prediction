...
def run_mlp(func, step, momentum, X, Z, TX, TZ, wd, opt, counter):
    ...
    typ = 'plain'
    if typ == 'plain':
        m = Mlp(input_size, [400, 100], 1, X, Z, hidden_transfers=func, out_transfer='identity', loss='squared', optimizer=optimizer, batch_size=batch_size, max_iter=max_iter)
...
if __name__ == '__main__':
    ...
    datafile = '/home/hpc/pr63so/ga93yih2/Dataset/qm7.pkl'
    ...
    while 1==1:
        ...
        for i in range(1, 60):
            if os.path.isfile(os.path.join(os.getcwd(),"hps"+str(i)+".pkl")):
                with open("hps"+str(i)+".pkl", 'rb') as tp:
                    func_ind1, step_ind1, momentum_ind1, wd_ind1, opt_ind1, counter1, n_iter1 = cp.load(tp)
                if (n_iter1 > 0) and (n_iter1 <= 25000000):
                    run_mlp(func_ind1, step_ind1, momentum_ind1, X, Z, TX, TZ, wd_ind1, opt_ind1, counter1)
        ...
        if (func_ind1 != funcs[func_ind]) or (step_ind1 != steps[step_ind]) or (momentum_ind1 != momentum[momentum_ind]) or (wd_ind1 != wds[wd_ind]) or (opt_ind1 != opts[opt_ind]):

            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                results = open('result_hp.txt', 'a')
                results.write('testing: %s,%s,%s,%s,%s\n' %(funcs[func_ind], steps[step_ind], momentum[momentum_ind], wds[wd_ind], opts[opt_ind]))
                results.close()
                run_mlp(funcs[func_ind], steps[step_ind], momentum[momentum_ind], X, Z, TX, TZ, wds[wd_ind], opts[opt_ind], counter)
